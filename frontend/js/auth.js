...
const page = {
    ...
    subtitle: isLogin ? 'Продолжите работу со своими чатами':'Один аккаунт для истории чатов',
    endpoint: isLogin ? '/api/auth/login' : 'api/auth/register',
    submit: isLogin ? 'Продолжить': 'Создать аккаунт',
    link: isLogin ? 'Нет аккаунта <a href="/register.html" class="font-medium text-ink underline">Зарегестрироваться</a>': 'Уже есть аккаунт? <a href="/login.html" class="font-medium text-ink underline">Войти</a>'
}
    ...
    <main class='auth-grid grid min-h-screen place-items-center px-5 py-10'>
        <div class='w-full max-w-sm'>
            ...
            <section class='rounded-2xl font-semibold border border-stone-200 bg-white p-7 shadow-soft'>
                ...
                <div id = 'formError' class='mt-4 hidden rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-sm text-red-700' role='alert'></div>
                <form id='authForm' class='space-y-4'>
                    ${nameField}
                        <label class="block">
                            <span class='mb-1.5 block text-xs font-semibold text-stone-700'>Email</span>
                            <input type='email' class='w-full h-11 rounded-lg border border-stone-300 px-3 text-sm focus:border-stone-700 focus:outline-none'
                            name='email'
                        ...
                        <label class="block">
                            ...
                            <span class='block relative'>
                                <input type='password' class='w-full h-11 rounded-lg border border-stone-300 px-3 text-sm focus:border-stone-700 focus:outline-none'
                                name='password'
                                ...
                                maxlength='128'
                                ${isLogin ? 'autofocus' : ''}/>
                                <button type='button' class='absolute inset-y-0 right-0 px-3 text-xs font-medium text-stone-500' id='passwordToggle'></button>
                            </span>
                            
                        </label>

                        <button type='summit' class='flex h-11 w-full items-center justify-center rounded-lg bg-stone-900 px-4 font-medium text-white hover:bg-black disabled:opacity-60'>${page.submit}</button>
...
passwordToggle.addEventListener('click', (ev) => {
    const input = form.nextElementSibling.password;
