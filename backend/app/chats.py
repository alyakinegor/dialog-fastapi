...
def required_chat(chat_id: int, user_id: int, db: DbSession):
...
async def list_chats(user: CurrentUser, db: DbSession):
    return list(db.scalar(select(Chat).where(Chat.user_id == user.id).order_by(Chat.updated_at.desc())))


@router.post('/chats', response_model=ChatResponse, status_code=status.HTTP_201_CREATED)
def create_chat(payload: CreateChatRequest, user: CurrentUser, db: DbSession):
...
async def send_message(chat_id: int, payload: SendMessageRequest, user: CurrentUser, db: DbSession):
    chat = required_chat(chat_id, user.id, db)
    recent = list(
        db.scalars(
